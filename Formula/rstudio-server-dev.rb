class RstudioServerDev < Formula
  desc "Integrated development environment (IDE) for R"
  homepage "https://www.rstudio.com"
  head "https://github.com/rstudio/rstudio.git", :using => :git
  ...
  if OS.linux?
    depends_on "boost-rstudio-server-dev"
  elsif OS.mac?
    depends_on "boost-rstudio-server-dev" => :build
  end
  depends_on "cmake" => :build
  depends_on "gcc" => :build
  depends_on :java => ["1.8", :build]
  depends_on "openssl"
  depends_on "soci-rstudio-server-dev"
  ...
  def install
    ...
    (common_dir/"node/10.19.0/").install resource("nodejs")
    
    # install yarn dependencies
    Dir.chdir(buildpath/"src/gwt/panmirror/src/editor")
    # NODE_BIN = common_dir/"node/10.19.0/bin"
    # INSTALL_PATH = "${NODE_BIN}:${PATH}"
    # TODO: add PATH = ${INSTALL_PATH} in front of yarn?
    ...
    mkdir "build" do
      ...
      args << "-DRSTUDIO_USE_SYSTEM_SOCI=Yes" # is this sufficient?
      args << "-DBoost_NO_SYSTEM_PATHS=On"
      args << "-DBOOST_ROOT=#{Formula["boost-rstudio-server-dev"].opt_prefix}"
      args << "-DCMAKE_INSTALL_PREFIX=#{prefix}/rstudio-server"
      args << "-DCMAKE_CXX_FLAGS=-std=c++14" # try forcing 14?
      
      # args << "-DCMAKE_CXX_FLAGS=-I#{Formula["openssl"].opt_include}"
      # # To avoid https://stackoverflow.com/questions/58628377/catalina-c-using-cmath-headers-yield-error-no-member-named-signbit-in-th
      # args << "-DCMAKE_CXX_FLAGS=-isystem /Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/include"
      # args << "-DCMAKE_CXX_FLAGS=-isystem /Applications/Xcode.app/Contents/Developer/Platforms/MacOSX.platform/Developer/SDKs/MacOSX.sdk"
      # args << "-DCMAKE_CXX_FLAGS=-isystem /Applications/Xcode.app/Contents/Developer/Platforms/MacOSX.platform/Developer/SDKs/MacOSX.sdk/usr/include"
      # TOOD: also need soci make_cxx flags?

      linkerflags = "-DCMAKE_EXE_LINKER_FLAGS=-L#{Formula["openssl"].opt_lib}"
      # TOOD: also need soci linker flags?
      if OS.linux?
